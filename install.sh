...
: "${USE_SUDO:="true"}"
: "${AURORA_INSTALL_DIR:="/usr/local/bin"}"
...
check_installed_version() {
  if [[ -f "${AURORA_INSTALL_DIR}/${APP_NAME}" ]]; then
    local version=$(aurorafs version 2>&1)
    if [[ "${version%-*}" == "${TAG#v}" ]]; then
      echo "Aurora ${version} is already ${DESIRED_VERSION:-latest}"
      return 0
    else
      echo "Aurora ${TAG} is available. Changing from version ${version}."
      return 1
    fi
  else
    return 1
...
# for that binary.
download_file() {
  AURORA_DIST="aurora-$OS-$ARCH"
  if [ "$OS" == "windows" ]; then
    AURORA_DIST="aurora-$OS-$ARCH.exe"
  fi
  DOWNLOAD_URL="$REPO_URL/releases/download/$TAG/$AURORA_DIST"
  AURORA_TMP_ROOT="$(mktemp -dt aurorafs-binary-XXXXXX)"
  AURORA_TMP_FILE="$AURORA_TMP_ROOT/$AURORA_DIST"
  if command -v curl &> /dev/null; then
    curl -SsL "$DOWNLOAD_URL" -o "$AURORA_TMP_FILE"
  elif command -v wget &> /dev/null; then
    wget -q -O "$AURORA_TMP_FILE" "$DOWNLOAD_URL"
  fi
}
...
# installs it.
install_file() {
  echo "Preparing to install $APP_NAME into ${AURORA_INSTALL_DIR}"
  run_as_root chmod +x "$AURORA_TMP_FILE"
  run_as_root cp "$AURORA_TMP_FILE" "$AURORA_INSTALL_DIR/$APP_NAME"
  echo "$APP_NAME installed into $AURORA_INSTALL_DIR/$APP_NAME"
}
...
test_binary() {
  if ! command -v $APP_NAME &> /dev/null; then
    echo "$APP_NAME not found. Is $AURORA_INSTALL_DIR on your "'$PATH?'
    exit 1
  fi
...
cleanup() {
  if [[ -d "${AURORA_TMP_ROOT:-}" ]]; then
    rm -rf "$AURORA_TMP_ROOT"
  fi
}
