...
func (t *TeeStore) Put(ctx context.Context, mode storage.ModePut, chs ...boson.Chunk) (exist []bool, err error) {
...
// Put implements storage.Putter.
func (f *FsStore) Put(ctx context.Context, mode storage.ModePut, chs ...boson.Chunk) (exist []bool, err error) {
	for _, ch := range chs {
...
func (f *FsStore) Get(ctx context.Context, mode storage.ModeGet, address boson.Address) (ch boson.Chunk, err error) {
	...
	return boson.NewChunk(address, data), nil
}

// ApiStore provies a storage.Putter that adds chunks to boson through the HTTP chunk API.
...
// Put implements storage.Putter.
func (a *ApiStore) Put(ctx context.Context, mode storage.ModePut, chs ...boson.Chunk) (exist []bool, err error) {
	for _, ch := range chs {
...
func (a *ApiStore) Get(ctx context.Context, mode storage.ModeGet, address boson.Address) (ch boson.Chunk, err error) {
	...
	ch = boson.NewChunk(address, chunkData)
	return ch, nil
}
