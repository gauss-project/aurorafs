...
func Split(cmd *cobra.Command, args []string) (err error) {
	...
	if useHttp {
		...
		stores.Add(store)
		logger.Debugf("using aurorafs http (ssl=%v) api on %s:%d for output", ssl, host, port)
	}
...
The application will expect to transmit the chunks to the aurorafs HTTP API, unless the --no-http flag has been set.
...
Chunks are saved in individual files, and the file names will be the hex addresses of the chunks.`,
	...
	c.Flags().BoolVar(&useHttp, "http", false, "save chunks to aurorafs http api")
