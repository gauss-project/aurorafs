...
func (ci *ChunkInfo) InitManifest() {
	fileListInfo, _ := ci.GetFileList(ci.addr)
	for _, v := range fileListInfo {
		rootCid := v["rootCid"].(string)
		maniFest, err := ci.ManifestView(context.TODO(), rootCid, "", 1)
		if err == nil {
			ci.manifest.Store(rootCid, maniFest)
		}
	}
}

func (ci *ChunkInfo) GetManifest(rootCid, pathVar string, depth int) (maniFest *ManifestNode) {
	mf, ok := ci.manifest.Load(rootCid)
	if !ok {
		maniFest, _ = ci.ManifestView(context.TODO(), rootCid, pathVar, depth)
		ci.manifest.Store(rootCid, maniFest)
	} else {
		maniFest = mf.(*ManifestNode)
	}
	return
}

func (ci *ChunkInfo) DelManifest(key string) {
	ci.manifest.Delete(key)
}

func (ci *ChunkInfo) ManifestView(ctx context.Context, nameOrHex, pathVar string, depth int) (*ManifestNode, error) {
	logger := tracing.NewLoggerWithTraceID(ctx, ci.logger)
	ls := loadsave.NewReadonly(ci.storer, storage.ModeGetRequest)

	address, err := ci.resolveNameOrAddress(nameOrHex)
	if err != nil {
	...
	if pathVar == "" || strings.HasSuffix(pathVar, "/") {
		...
		fn := func(nodeType int, path, prefix, hash []byte, metadata map[string]string) error {
			...
			case int(manifest.File):
				...
				refChunk, err := ci.storer.Get(ctx, storage.ModeGetRequest, boson.NewAddress(hash))
				if err != nil {
					return err
				}
		...
		refChunk, err := ci.storer.Get(ctx, storage.ModeGetRequest, e.Reference())
		if err != nil {
...
func (ci *ChunkInfo) resolveNameOrAddress(str string) (boson.Address, error) {
	log := ci.logger
	...
	addr, err = ci.resolver.Resolve(str)
	if err == nil {
