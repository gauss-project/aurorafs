...
func (s *PSlice) Add(addr boson.Address, po uint8) {
	s.Lock()
	defer s.Unlock()
	...
	tail := append([]boson.Address{addr}, peers[s.bins[po]:]...)
...
func (s *PSlice) Remove(addr boson.Address, po uint8) {
	s.Lock()
	defer s.Unlock()
...
func (s *PSlice) copy() (peers []boson.Address, bins []uint) {
	peers = make([]boson.Address, len(s.peers))
	copy(peers, s.peers)
