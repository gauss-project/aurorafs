...
func TestShallowestEmpty(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		for _, v := range peers[tc.removePo] {
			po := boson.Proximity(base.Bytes(), v.Bytes())
			ps.Remove(v, po)
		}
		sd, none := ps.ShallowestEmpty()
...
func TestIteratorsJumpStop(t *testing.T) {
	...
	testIterator(t, ps, true, false, 4, []boson.Address{peers[11], peers[8], peers[5], peers[2]})
	testIteratorRev(t, ps, true, false, 4, []boson.Address{peers[2], peers[5], peers[8], peers[11]})

	// check that the stop functionality works correctly
	testIterator(t, ps, true, true, 1, []boson.Address{peers[11]})
	testIteratorRev(t, ps, true, true, 1, []boson.Address{peers[2]})
