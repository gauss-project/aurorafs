...
func (r *peerRegistry) addIfNotExists(c network.Conn, overlay boson.Address) (exists bool) {
	peerID := c.RemotePeer()
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *peerRegistry) remove(overlay boson.Address) (bool, libp2ppeer.ID) {
	r.mu.Lock()
	peerID, found := r.underlays[overlay.ByteString()]
	delete(r.overlays, peerID)
	delete(r.underlays, overlay.ByteString())
