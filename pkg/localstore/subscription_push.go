...
import (
	...
	"github.com/gauss-project/aurorafs/pkg/flipflop"
...
func (db *DB) SubscribePush(ctx context.Context) (c <-chan boson.Chunk, stop func()) {
	db.metrics.SubscribePush.Inc()

	chunks := make(chan boson.Chunk)
		...
		for {
			...
			case <-out:
				...
				err := db.pushIndex.Iterate(func(item shed.Item) (stop bool, err error) {
					...
					select {
					case chunks <- boson.NewChunk(boson.NewAddress(dataItem.Address), dataItem.Data).WithTagID(item.Tag):
						count++
