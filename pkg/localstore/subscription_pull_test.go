...
func TestDB_SubscribePull_first(t *testing.T) {
	...
	addrs := make(map[uint8][]boson.Address)
...
func TestDB_SubscribePull(t *testing.T) {
	...
	for bin := uint8(0); bin <= boson.MaxPO; bin++ {
		...
		for i := 0; i < count; i++ {
			...
			if wanted {
				if _, ok := addrs[bin]; !ok {
					addrs[bin] = make([]boson.Address, 0)
				}
...
func uploadRandomChunksBin(t *testing.T, db *DB, addrs map[uint8][]boson.Address, addrsMu *sync.Mutex, wantedChunksCount *int, count int) {
	addrsMu.Lock()
	defer addrsMu.Unlock()
...
func readPullSubscriptionBin(ctx context.Context, db *DB, bin uint8, ch <-chan storage.Descriptor, addrs map[uint8][]boson.Address, addrsMu *sync.Mutex, errChan chan error) {
...
func TestAddressInBin(t *testing.T) {
	...
	for po := uint8(0); po < boson.MaxPO; po++ {
