...
func (k *Kad) connectionAttemptsHandler(ctx context.Context, wg *sync.WaitGroup, peerConnChan, peerConnChan2 <-chan *peerConnInfo) {
	connect := func(peer *peerConnInfo) {
		...
		//check if this one is in a full bin
		if len(k.connectedPeers.PeersSameBin(peer.addr)) > saturationPeers{
			k.logger.Warningf("kademlia: bin of %q is full", peer.addr)
			return
		}
...
func (k *Kad) notifyManageLoop() {
	...
	if k.connectCount < 5 {
		...
		k.logger.Infof("Ignore connect out of %v",5)
	}
	k.connectMu.Unlock()
