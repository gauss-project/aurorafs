...
type swapProtocolMock struct {
	emitCheque func(ctx context.Context, peer boson.Address, cheque *chequebook.SignedCheque) error
}

func (m *swapProtocolMock) EmitCheque(ctx context.Context, peer boson.Address, cheque *chequebook.SignedCheque) error {
	if m.emitCheque != nil {
...
type addressbookMock struct {
	...
	chequebook      func(peer boson.Address) (chequebookAddress common.Address, known bool, err error)
	beneficiaryPeer func(beneficiary common.Address) (peer boson.Address, known bool, err error)
	chequebookPeer  func(chequebook common.Address) (peer boson.Address, known bool, err error)
	putBeneficiary  func(peer boson.Address, beneficiary common.Address) error
	putChequebook   func(peer boson.Address, chequebook common.Address) error
}
...
func (m *addressbookMock) Chequebook(peer boson.Address) (chequebookAddress common.Address, known bool, err error) {
...
func (m *addressbookMock) ChequebookPeer(chequebook common.Address) (peer boson.Address, known bool, err error) {
...
func (m *addressbookMock) PutChequebook(peer boson.Address, chequebook common.Address) error {
...
func TestReceiveCheque(t *testing.T) {
	...
	addressbook := &addressbookMock{
		chequebook: func(p boson.Address) (common.Address, bool, error) {
			if !peer.Equal(p) {
				t.Fatal("querying chequebook for wrong peer")
			}
			return chequebookAddress, true, nil
		},
		putChequebook: func(p boson.Address, chequebook common.Address) (err error) {
			if !peer.Equal(p) {
...
func TestPay(t *testing.T) {
	...
	swap := swap.New(
		&swapProtocolMock{
			emitCheque: func(ctx context.Context, p boson.Address, c *chequebook.SignedCheque) error {
				if !peer.Equal(p) {
...
func TestHandshake(t *testing.T) {
	...
	swapService := swap.New(
		...
		&addressbookMock{
			...
			putBeneficiary: func(p boson.Address, b common.Address) error {
				putCalled = true
				return nil
			},
		},
