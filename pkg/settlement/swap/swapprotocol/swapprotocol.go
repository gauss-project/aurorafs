...
type Interface interface {
	...
	EmitCheque(ctx context.Context, peer boson.Address, cheque *chequebook.SignedCheque) error
}

// Swap is the interface the settlement layer should implement to receive cheques.
type Swap interface {
	// ReceiveCheque is called by the swap protocol if a cheque is received.
	ReceiveCheque(ctx context.Context, peer boson.Address, cheque *chequebook.SignedCheque) error
	// Handshake is called by the swap protocol when a handshake is received.
	Handshake(peer boson.Address, beneficiary common.Address) error
}
...
// EmitCheque sends a signed cheque to a peer.
func (s *Service) EmitCheque(ctx context.Context, peer boson.Address, cheque *chequebook.SignedCheque) error {
	ctx, cancel := context.WithTimeout(ctx, 5*time.Second)
	defer cancel()
