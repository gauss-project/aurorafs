...
//
//func (s *server) checkPin(w http.ResponseWriter,r *http.Request){
//	addr, err := boson.ParseHexAddress(mux.Vars(r)["address"])
//	if err != nil {
//		s.logger.Debugf("pin files: parse address: %v", err)
//		s.logger.Error("pin files: parse address")
...
//
//	ctx := r.Context()
//	var addlist []boson.Address
//	getChunkList := func(address boson.Address)error{
//		addlist = append(addlist, address)
//		return nil
//	}
//
//	err = s.traversal.TraverseFileAddresses(ctx, addr, getChunkList)
//	if err != nil {
//		s.logger.Debugf("pin files: traverse chunks: %v, addr %s", err, addr)
//
//		if errors.Is(err, traversal.ErrInvalidType) {
