...
func TestAuroraFiles(t *testing.T) {
	var (
		...
		targets              = "0x222"
		fileDownloadResource = func(addr string) string { return "/aurora/" + addr }
		simpleData           = []byte("this is a simple text")
		storerMock           = smock.NewStorer()
		pinningMock          = pinning.NewServiceMock()
		traverser            = traversal.New(storerMock)
		chunkinfoMock        = chunkinfo.New(routetab.NewMockRouteTable())
		logger               = logging.New(ioutil.Discard, 0)
		client, _, _         = newTestServer(t, testServerOptions{
			ChunkInfo: chunkinfoMock,
			Storer:    storerMock,
			Pinning:   pinningMock,
			Traversal: traverser,
			Logger:    logger,
	...
	t.Run("tar-file-upload", func(t *testing.T) {
		...
		reference := boson.MustParseHexAddress("173b343710c42d9b57fbbb44129a0d7f7f9d56656a20a4432102fa3e1391c49c")
	...
	t.Run("upload-then-download-and-check-data", func(t *testing.T) {
		...
		rootHash := "b5e302c4b2cd58e49ecbf06c099f53b9da3bece038b94041e9df47d34898d290"
	...
	t.Run("upload-then-download-with-targets", func(t *testing.T) {
		...
		rootHash := "c8e81c6979fd85ec945e88f999aeb84c4c12db86f7f6fb9629629eb3bf8fbd8a"
...
func TestAuroraFilesRangeRequests(t *testing.T) {
	...
	uploads := []struct {
		...
		{
			name:             "file",
			uploadEndpoint:   "/aurora",
			downloadEndpoint: "/aurora",
			reader:           bytes.NewReader(data),
			contentType:      "text/plain; charset=utf-8",
		},
		{
	...
	for _, upload := range uploads {
		t.Run(upload.name, func(t *testing.T) {
			...
			if upload.name == "dir" {
				testOpts = append(testOpts, jsonhttptest.WithRequestHeader(api.AuroraCollectionHeader, "True"))
			}
