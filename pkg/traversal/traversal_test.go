...
func TestTraversalBytes(t *testing.T) {
	traverseFn := func(traversalService traversal.Service) func(context.Context, boson.Address, boson.AddressIterFunc) error {
	...
	testCases := []struct {
		...
		{
			dataSize:            boson.ChunkSize*129 - 1,
			expectedHashesCount: 131,
			expectedHashes: []string{
