...
type Interface interface {
	...
	Reserve(ctx context.Context, peer boson.Address, price uint64) error
	// Release releases the reserved funds.
	Release(peer boson.Address, price uint64)
	// Credit increases the balance the peer has with us (we "pay" the peer).
	Credit(peer boson.Address, price uint64) error
	// Debit increases the balance we have with the peer (we get "paid" back).
	Debit(peer boson.Address, price uint64) error
	// Balance returns the current balance for the given peer.
	Balance(peer boson.Address) (*big.Int, error)
	// SurplusBalance returns the current surplus balance for the given peer.
	SurplusBalance(peer boson.Address) (*big.Int, error)
	...
	CompensatedBalance(peer boson.Address) (*big.Int, error)
...
// Reserve reserves a portion of the balance for peer and attempts settlements if necessary.
func (a *Accounting) Reserve(ctx context.Context, peer boson.Address, price uint64) error {
...
func (a *Accounting) Release(peer boson.Address, price uint64) {
	accountingPeer, err := a.getAccountingPeer(peer)
	if err != nil {
...
func (a *Accounting) Credit(peer boson.Address, price uint64) error {
	accountingPeer, err := a.getAccountingPeer(peer)
	if err != nil {
		return err
	}
...
// called.
func (a *Accounting) settle(ctx context.Context, peer boson.Address, balance *accountingPeer) error {
...
// existing credit).
func (a *Accounting) Debit(peer boson.Address, price uint64) error {
	accountingPeer, err := a.getAccountingPeer(peer)
	if err != nil {
		return err
	}
...
// Balance returns the current balance for the given peer.
func (a *Accounting) Balance(peer boson.Address) (balance *big.Int, err error) {
...
func (a *Accounting) SurplusBalance(peer boson.Address) (balance *big.Int, err error) {
...
func (a *Accounting) CompensatedBalance(peer boson.Address) (compensated *big.Int, err error) {
...
func peerSurplusBalanceKey(peer boson.Address) string {
...
func (a *Accounting) NotifyPaymentThreshold(peer boson.Address, paymentThreshold *big.Int) error {
	accountingPeer, err := a.getAccountingPeer(peer)
	if err != nil {
		return err
	}
